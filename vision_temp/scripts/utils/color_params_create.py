...
while(not esc):
	...
	while(1):
		...
		if key == ord('r') or key == ord('g') or key == ord('y'):
			...
			print("training svm")
			svm_params = dict( kernel_type = cv2.SVM_SIGMOID,
                    svm_type = cv2.SVM_ONE_CLASS,
                    gamma=0.1,nu=0.1,coef0=0.1)
			svm = cv2.SVM()
			svm.train(np.asarray(red_hue_cor, dtype=np.float32),np.ones(len(red_hue_cor)), params=svm_params)
			svm.save('svm_red.xml')

			svm = cv2.SVM()
			svm.train(np.asarray(green_hue_cor, dtype=np.float32),np.ones(len(green_hue_cor)), params=svm_params)
			svm.save('svm_green.xml')

			svm = cv2.SVM()
			svm.train(np.asarray(yellow_hue_cor, dtype=np.float32),np.ones(len(yellow_hue_cor)), params=svm_params)
